...
def worker(num):
    ...
    # set the output path
    prms['path'] = 'simulations/mature_s1/'
    ...
    prms['spatio_temp_corr'] = 0.5 #numpy.random.uniform(low=0.0,high=1.0) # 0.5 #numpy.random.uniform(low=0.0,high=1.0)
    prms['logFlag'] = (num == 0)
    prms['bias_s1'] = numpy.random.uniform(low=0.05,high=0.4)
    prms['spread_s1'] = 4
    ...
    prms['W_initial_s1'] = [Uexp - 0.1, Uexp + 0.1] 
    mats['W_s1'] = biased_weights(prms['N_s1'], prms['W_initial_s1'], prms['bias_s1'], prms['spread_s1'])
    # run the simulation
