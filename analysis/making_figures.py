...
def making_figures(path, W_thres, W_v1, W_s1, W_vec_v1, W_vec_s1, time_vec_list):

    fig = plt.subplots(1, 2, figsize=(13, 5))
    plt.subplot(1, 2, 1)
    im = plt.imshow(W_v1, vmin=W_thres[0], vmax=W_thres[1]/1, cmap='magma')
    plt.colorbar(im, fraction=0.046, pad=0.04)
    plt.tight_layout()
    plt.xlabel('V1')
    plt.ylabel('RL')
    plt.subplot(1, 2, 2)
    im = plt.imshow(W_s1, vmin=W_thres[0], vmax=W_thres[1]/1, cmap='magma')
    plt.colorbar(im, fraction=0.046, pad=0.04)
    plt.tight_layout()
    plt.xlabel('S1')
    plt.ylabel('RL')
    ...
    for ii in range(25):
        ...
        plt.imshow(W_vec_v1[ii][:][:], vmin=W_thres[0], vmax=W_thres[1], cmap='magma')
    ...
    for ii in range(25):
        plt.subplot(5, 5, ii + 1)
        plt.imshow(W_vec_s1[ii][:][:], vmin=W_thres[0], vmax=W_thres[1], cmap='magma')
        plt.title('Time = ' + str(time_vec_list[ii]/1000))
        plt.axis('off')
